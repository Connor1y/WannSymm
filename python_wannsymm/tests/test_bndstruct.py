...
class TestDiagonalizeHamiltonian:
    ...
    def test_simple_identity(self):
        ...
        np.testing.assert_allclose(vec @ vec.T.conj(), np.eye(2), atol=1e-10)
...
class TestWriteBandCharacters:
    ...
    def test_write_basic(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ndegen = np.array([1, 1])
            sym_chas = [np.array([1.0 + 0j, 1.0 + 0j]), np.array([1.0 + 0j, -1.0 + 0j])]
...
class TestWriteBandEigenvalues:
    ...
    def test_write_basic(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            eig_hk = np.array([1.0, 2.0])
            ndegen = np.array([1, 1])
            sym_eigs = [np.array([1.0 + 0j, 1.0 + 0j]), np.array([1.0 + 0j, -1.0 + 0j])]
...
class TestWriteBands:
    ...
    def test_write_single_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ebands = np.array([[1.0, 2.0], [1.1, 2.1], [1.2, 2.2]])
    ...
    def test_write_multiple_paths(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ebands = np.array([[1.0, 2.0], [1.5, 2.5], [1.5, 2.5], [2.0, 3.0]])
