...
from .constants import MAXLEN, eps8
...
class WannData:
    ...
    def __post_init__(self):
        ...
        if self.norb > 0 and self.nrpt > 0:
            ...
            if self.ham.size == 0:
                self.ham = np.zeros((self.nrpt, self.norb, self.norb), dtype=np.complex128)
            if self.hamflag.size == 0:
                self.hamflag = np.zeros((self.nrpt, self.norb, self.norb), dtype=np.int32)
            if len(self.rvec) == 0:
    ...
    with open(filename, 'r') as fin:
        ...
        for irpt in range(nrpt):
            for ielement in range(norb * norb):
                ...
                iorb_left = int(parts[3]) - 1  # Convert to 0-based indexing (jorb in C write)
                iorb_right = int(parts[4]) - 1  # Convert to 0-based indexing (iorb in C write)
                real_part = float(parts[5])
                imag_part = float(parts[6])
                
                # Store R-vector (only once per R-point, when both indices are 0)
                if iorb_right == 0 and iorb_left == 0:
                    wann.rvec[irpt] = Vector(t1, t2, t3)
                
                # Store Hamiltonian element
                # In file: columns 4,5 are (jorb+1, iorb+1) from C write loop
                # C writes: for(iorb) for(jorb) fprintf(..., jorb+1, iorb+1, ham[iorb][jorb])
                # So iorb_left=jorb, iorb_right=iorb in the C code
                # C stores as: ham[irpt*norb*norb + iorb*norb + jorb]
                # Python equivalent: ham[irpt, iorb, jorb] = ham[irpt, iorb_right, iorb_left]
                wann.ham[irpt, iorb_right, iorb_left] = real_part + 1j * imag_part
